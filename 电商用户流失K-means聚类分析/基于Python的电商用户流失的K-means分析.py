...
def find_optimal_k(X, k_range=None):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'optimal_k_analysis.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
def visualize_clusters_2d(X, labels, centers, feature_names, title="用户聚类结果"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'clustering_visualization.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
def visualize_cluster_profiles(df, labels, feature_cols):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'cluster_profiles.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
def visualize_cluster_summary(df, labels, feature_cols):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, 'cluster_radar.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # 配置参数
    DATA_PATH = '/Users/caojun/Desktop/Data-analysis/Python电商购买行为分析及建模/customerData_500k.csv'
    ...
    df_result.to_csv(os.path.join(SCRIPT_DIR, 'user_clustering_results.csv'), index=False)
